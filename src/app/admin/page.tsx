  ...
  const fetchStats = async () => {
    try {
      ...
      if (productsData.success && Array.isArray(productsData.data)) {
        ...
        const categories = categoriesData.success ? categoriesData.data.categories.length : 
                          new Set(products.map((p: Product) => p.category)).size

        setStats({
          totalProducts: products.length,
          totalCategories: categories,
          totalNews: newsData.success ? newsData.data?.length || 0 : 0,
          totalEvents: eventsData.success ? eventsData.data?.length || 0 : 0
        })
